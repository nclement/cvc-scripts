...
double best_of_rounded_bardelta(int *xn_minus, int *xn_extraminus, int *xc_index, int current_iteration)
{
  double fxn_minus;
...
double oldmain(double **pointset, int n, int d) {
  ...
  double global[trials+1], best, mean;  //current and global best values
  ...
  for(int t = 1; t <= trials; t++) {
    ...
    // TODO: Can parallelize this loop (should be simple: no writing)
    ...
    for(int i = 1; i <= outerloop; i++) {
      ...
      double current = best_of_rounded_bardelta(xn_minus_index, xn_extraminus_index, xc_index, current_iteration);

      //draw a neighbour of xc
      generate_neighbor_bardelta(xn_minus_index, xn_extraminus_index, xc_index, k, mc);

      //Compute the threshold
      double fxc = best_of_rounded_bardelta(xn_minus_index, xn_extraminus_index, xc_index, current_iteration);
      thresh[i]=0.0-fabs(fxc-current);
    }	

    // TODO: This should be done by a single thread.
    //sort the thresholds in increasing order
    quicksort(1,outerloop,thresh);

    // TODO: Set these values to zero here (only one thread)
    switches[t]=0;
    global_switches[t]=0;
    ...
    // TODO: each thread should set its own random initial point.
    ...
    current_iteration = 0;
    for(int i = 1; i <= outerloop; i++) {
      double T = thresh[i];    //current Threshold

      for(int p = 1; p <= innerloop; p++) {
        current_iteration++;
...
#endif
#ifdef PRINT_ALL_UPDATES
          fprintf(stderr, "shrink bardelta %g at %d :", fxc, g_current_iteration);
          for (int j = 0; j < d; j++)

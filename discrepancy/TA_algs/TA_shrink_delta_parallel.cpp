...
double g_real_max_discr=0;
int g_real_when=0, g_when=0;
int g_current_iteration;
...
double best_of_rounded_delta(int *xn_plus)
{
  ...
  fprintf(stderr, "PRIVATE candidate %g (vs %g)\n",
	  fxc, g_real_max_discr);
#endif
  if (fxc > g_real_max_discr) {
    g_real_max_discr = fxc;
    g_real_when = g_current_iteration;
#ifdef PRINT_ALL_UPDATES
    fprintf(stderr, "Secret update at %d to %g\n",
	    g_current_iteration, fxc);
#endif
  }
#endif
...
double oldmain(double **pointset, int n, int d)
{
  ...
  for(t=1;t<=trials;t++)
	...
	//Update k-value
	  for (j=0; j<d; j++) {
	    k[j] = start[j]*(((double)outerloop-g_current_iteration)/(outerloop)) +
	      1*((double)g_current_iteration/(outerloop));
		  //	    k[j]=start[j] - (int)((3.0/4)*(g_current_iteration/outerloop)*(start[j]-1));
	  }

        //Update mc-value
	  mc=2+(int)(g_current_iteration/outerloop*(d-2));
	...
	start[j]=(int)((g_n_coords[j]-1)/2);
      ...
      //Initialize mc-value
      mc=2+(int)(g_current_iteration/(innerloop*outerloop)*(d-2));
      ...
      for(i=1;i<=outerloop;i++)
	{
	  ...
	  for(p=1;p<=innerloop;p++)
...
#endif
	      for (j=0; j<d; j++) {
		k[j] = start[j]*(((double)innerloop*outerloop-g_current_iteration)/(innerloop*outerloop)) +
		  1*((double)g_current_iteration/(innerloop*outerloop));
		  //		k[j]=(int)(start[j]-(int)(g_current_iteration/(innerloop*outerloop)*(start[j]-1)));
#ifdef PRINT_RANGE_DATA
		if (p==1)
		...
		global_switches[t]++;
		global[t]=fxc;
		g_when=g_current_iteration;
#ifdef PRINT_UPDATE_CANDIDATES
		fprintf(stderr, "global ");
#endif
#ifdef PRINT_ALL_UPDATES
		fprintf(stderr, "shrink delta %g at %d :", fxc, g_current_iteration);
		for (j=0; j<d; j++)
		  fprintf(stderr, " %d", xn_plus_index[j]);
		fprintf(stderr, "\n");
#endif
      ...
      if (g_real_max_discr > global[t]) {
	global[t] = g_real_max_discr;
	g_when = g_real_when;
	//	fprintf(stderr, "Max value subsumed\n");
      }
      fprintf(stderr, "shrink delta Result %g at %d\n", global[t], g_when);
...
int main(int argc, char **argv)
{
  ...
  pointset = (double**)malloc(npoints*sizeof(double*));
  for (i=0; i<npoints; i++) {

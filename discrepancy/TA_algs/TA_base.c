...
double best_of_rounded_base(int *xn_plus, int *xn_minus, int *xn_extraminus, int *xc_index)
{
  double fxn_plus;
  double fxn_minus;
  ...
  int xn_plus_grow[d], xn_minus_snap[d], xn_extraminus_snap[d];
#endif
  for (j=0; j<d; j++)
...
#if (VERSION == 1)
  fxn_plus = get_delta(xn_plus_grow);
  fxn_minus= get_bar_delta(xn_minus_snap);
  fxc=max(fabs(fxn_plus), fabs(fxn_minus));
  if (use_extraminus) {
  ...
  fxn_plus = get_delta(xn_plus);
  fxn_minus= get_bar_delta(xn_minus);
  fxc=max(fabs(fxn_plus), fabs(fxn_minus));
  if (use_extraminus) {
  ...
  if (use_extraminus && (fxn_extraminus > fxn_plus) && (fxn_extraminus > fxn_minus)) {
    for (j=0; j<d; j++)
      xc_index[j] = xn_extraminus[j];
  }
  else if (fxn_plus > fxn_minus) {
    for(j=0; j<d; j++) 
	xc_index[j]= xn_plus[j];
  }
  else { 
    for(j=0; j<d; j++) 
...
double oldmain(double **pointset, int n, int d)
{
  ...
  double fxc;
  int xc_index[d], xn_plus_index[d], xn_minus_index[d], xn_extraminus_index[d];
  ...
  for(t=1;t<=trials;t++)
	...
	generate_xc(xn_plus_index, xn_minus_index, xn_extraminus_index); 
	
	//(Possibly) Snap the points and compute the largest of the rounded values 
	current = best_of_rounded_base(xn_plus_index, xn_minus_index, xn_extraminus_index, xc_index);
    
	//draw a neighbour of xc
	generate_neighbor(xn_plus_index, xn_minus_index, xn_extraminus_index, xc_index, k, mc);
	
	//Compute the threshold
	fxc=best_of_rounded_base(xn_plus_index, xn_minus_index, xn_extraminus_index, xc_index);
      ...
      for(i=1;i<=outerloop;i++)
	{
	  ...
	  for(p=1;p<=innerloop;p++)
		...
		fprintf(stderr, "%d ", xn_extraminus_index[j]);
	      ...
	      fxc = best_of_rounded_base(xn_plus_index, xn_minus_index, xn_extraminus_index, xn_best_index);
#ifdef PRINT_UPDATE_CANDIDATES

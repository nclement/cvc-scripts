...
GEN_PARAM=${DOCKING_SCRIPTS_DIR}/gen-param-file_coarse.sh
...
# lower-quality models for surface area (shorter running time)
$LIGAND $LIG 128
$RECEPTOR $REC 128
echo $GEN_PARAM $LIG $REC $TYPE $OUT $gridSpacing $rotFile \> ${OUT%.txt}.inp
$GEN_PARAM $LIG $REC $TYPE $OUT $gridSpacing $rotFile > ${OUT%.txt}.inp
if [[ "$#" -eq "4" ]]; then
